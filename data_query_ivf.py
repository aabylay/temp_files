...
def queryVector(vector, k, value=None):
    ...
    res_ivf_10_1 = test_imdb_ivfflat_10.search(
        data=[vector],
        anns_field="storyline_vec", 
        param=search_params,
        limit=k,
    ...
    q_time_10_1, start = time.time() - start, time.time()
    ...
    res_ivf_10_3 = test_imdb_ivfflat_10.search(
        data=[vector],
        anns_field="storyline_vec", 
        param=search_params,
        limit=k,
    ...
    q_time_10_3, start = time.time() - start, time.time()
    ...
    res_ivf_10_10 = test_imdb_ivfflat_10.search(
        data=[vector],
        anns_field="storyline_vec", 
        param=search_params,
        limit=k,
    ...
    q_time_10_10, start = time.time() - start, time.time()
    
    #for res in res_hnsw_s[0]:
    #    print(type(res))
        
    return [\
        res_ivf_10_1[0].ids, res_ivf_10_3[0].ids, res_ivf_10_10[0].ids, res_ivf_100_1[0].ids, res_ivf_100_10[0].ids, \
        q_time_10_1, q_time_10_3, q_time_10_10, q_time_100_1, q_time_100_10 \
    ]
...
columns = ["id", "prompt", "k", "selection", \
           "ivf10_1_runtime", "ivf10_1_recall",
           "ivf10_3_runtime", "ivf10_3_recall",
           "ivf10_10_runtime", "ivf10_10_recall",
...
for i in range(df_top_res.shape[0]):
    ...
    for k in k_vals.keys():
        ...
        recall_ivf_10_1 = len(set(results[0]).intersection(true_res)) / len(true_res)
        recall_ivf_10_3 = len(set(results[1]).intersection(true_res)) / len(true_res)
        recall_ivf_10_10 = len(set(results[2]).intersection(true_res)) / len(true_res)
        ...
        df_temp = pd.DataFrame([[title_id1, "top_100", k, val1, \
                            results[5], recall_ivf_10_1, \
                            results[6], recall_ivf_10_3, \
                            results[7], recall_ivf_10_10, \
                            results[8], recall_ivf_100_1, \
                            results[9], recall_ivf_100_10]], \
                            columns=columns
                            )
...
df.to_csv("results_df_ivf_full_idx.csv")
print("Done")
